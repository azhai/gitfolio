...
import { Box, Text, Input, Textarea, Button, Flex, Switch, useToast } from '@chakra-ui/react'
...
const MigrateProject = () => {
  ...
  function handleSubmit() {
    ...
    if (!form.clone_url.trim()) {
      toast({ title: '请输入远程仓库 URL', status: 'error', duration: 3000 })
      return
    }
    setSubmitting(true)
    reposAPI.create(form).then(function(data) {
      ...
      toast({ title: err.message || '迁移项目失败', status: 'error', duration: 3000 })
    }).finally(function() { setSubmitting(false) })
  }

  return (
    <Box maxW="720px" mx="auto">
      <Text fontSize="22px" fontWeight="700" color="#333" mb="24px">迁移项目</Text>

      <Box bg="white" border="1px solid" borderColor="#e2e2e2" rounded="10px" p="28px">
        <Box mb="18px">
          <Text fontSize="13.5px" fontWeight="600" color="#555" mb="6px">远程仓库 URL *</Text>
          ...
          <Text fontSize="12px" color="#999" mt="4px">输入要迁移的 Git 仓库地址</Text>
        </Box>
        ...
        <Flex align="center" justify="space-between" mb="24px" py="10px" borderBottom="1px solid" borderColor="#f0f0f0">
          <Box>
            <Text fontSize="13.5px" fontWeight="500" color="#555">镜像模式</Text>
            <Text fontSize="12px" color="#888">自动从远程仓库同步更新</Text>
          </Box>
          <Switch colorScheme="green" isChecked={form.is_mirror} onChange={updateField('is_mirror')} />
        </Flex>
